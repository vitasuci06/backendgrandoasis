...
if (isset($_POST['login'])) {
    ...
    if (!$email && !$password) {
        ...
        $query = "SELECT * FROM admin WHERE Username = '$email' OR Email='$email' AND Password='$password'";
...
if (isset($_POST['cutomerDetails'])) {
    ...
    if ($reservasi_id != '') {
        $sql = "SELECT * FROM reservasikamar NATURAL JOIN tamu NATURAL JOIN kamar WHERE ReservasiKamar_id = $reservasi_id";
        $result = mysqli_query($connection, $sql);
        if ($result) {
            ...
            $response['Tipe_kamar'] = $customer_details['Tipe_kamar'];
            $response['Nomor_kamar'] = $customer_details['Nomor_kamar'];
            $response['CheckInDate'] = $customer_details['CheckInDate'];
            $response['CheckOutDate'] = $customer_details['CheckOutDate'];
            $response['Status'] = $customer_details['Status'];
            $response['Harga'] = $customer_details['Harga'];
            $response['Tanggal_bayar'] = $customer_details['Tanggal_bayar'];
            $response['Jumlah_bayar'] = $customer_details['Jumlah_bayar'];
            $response['Metode_bayar'] = $customer_details['Metode_bayar'];
        } else {
            $response['done'] = false;
            $response['data'] = "DataBase Error";
        }
